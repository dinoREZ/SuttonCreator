...
public class ${primaryName}${secondaryName}DaoHibernateImpl implements ${primaryName}${secondaryName}DaoHibernate {
    ...
    @Override
    public CollectionModelHibernateResult<${secondaryName}DB> readBy<#list query.queryParameters as queryParameter>${queryParameter.name?cap_first}</#list>(long primaryId, <#list query.pathQueryParameters as queryParameter>${queryParameter.type} ${queryParameter.name}, </#list>SearchParameter searchParameter) {
        return new  ${primaryName}${secondaryName}QueryBy<#list query.queryParameters as queryParameter>${queryParameter.name?cap_first}</#list>Operation(emf, primaryId, <#list query.pathQueryParameters as queryParameter>${queryParameter.name}, </#list>searchParameter).start();
    }
