...
public interface ${name}DaoHibernate extends IDatabaseAccessObjectHibernate<${name}DB> {
    ...
    CollectionModelHibernateResult<${name}DB> readBy<#list query.attributes as attributeTriple>${attributeTriple.middle?cap_first}</#list>(<#list query.attributes as attributeTriple>${attributeTriple.left} ${attributeTriple.middle}, </#list>SearchParameter searchParameter);
    </#list>
